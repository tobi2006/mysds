...
def invite_students(request):
    if request.method == 'POST':
        ...
        for student_id in student_ids:
            ...
            student_group.save()
            first_name = student.first_name.split(" ")[0]
            message = """
Dear %s,

You now have access to MySDS, the Student Database for Law at Canterbury
Christ Church University.

The database is available at https://cccu.tobiaskliem.de, and you can access
...
On the database, you will be able to see your marks, download your marksheets
and book meetings with teachers. The first marksheets should be up soon, and
your teachers will let you know about this.
...
""" % (first_name, user.username, password)
            send_mail(
                'The Law Database at Canterbury Christ Church',
                message,
                'cccu@tobiaskliem.de',
                [user.email, ]
                )
...
def wrong_email(request):
    printstring = '''
<p>You did not provide a valid email address.</p>
<p>
    Please make sure you use your CCCU email address
    - eg b.bunny123@canterbury.ac.uk.
</p>
